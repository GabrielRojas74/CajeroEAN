    ...
    def Parqueadero():
      ...
      def opcionespar():
        ...
        def cambiospues():
          ...
          retiro = Label(vueltas,text="¿Cuanto desea retirar? ", font=("Bahnschrift SemiBold Condensed", 15, BOLD), bg="purple", fg="#271F26", width="40", height=2, bd=8)
          retiro.pack()
          
          dinerovueltas= Label(vueltas, text="el dinero de las vueltas es ", font=("Bahnschrift SemiBold Condensed", 15, BOLD), bg="purple", fg="#271F26", width="40", height=2, bd=8, relief=RAISED)
          dinerovueltas = int(entrada4.get()) 
          vueltos = Label(vueltas,text=str(entrada4.get())) 
          vueltos.pack(pady=25)
          
          dinero_de_caja = int(entrada3.get()) 
          reitross=int(entrada4.get())
          if (reitross<=dinero_de_caja):
            if (reitross >= 100_000):
              queda = reitross()//100_000
              print(str(queda) + "billete\s de 100_000 pesos")
              reitross = reitross % 100_000
            if (reitross >= 50_000):
              queda = reitross//50_000
              print(str(queda) + "billete\s de 50_000 pesos")
              reitross = reitross % 50_000
            if (reitross >= 20_000):
              queda = reitross//20_000
              print(str(queda) + "billete\s de 20_000 pesos")
              reitross = reitross % 20_000
            if (reitross >= 10_000):
              queda = reitross//10_000
              print(str(queda) + "billete\s de 10_000 pesos")
              reitross = reitross % 10_000
            if (reitross >= 5_000):
              queda = reitross//5_000
              print(str(queda) + "billete\s de 5_000 pesos")
              reitross = reitross % 5_000
            if (reitross >= 1_000):
              queda = reitross//1_000
              print(str(queda) + "billete\s de 1_000 pesos")
              reitross = reitross % 1_000
            if (reitross >= 500):
              queda = reitross//500
              print(str(queda) + "moneda\s de 500 pesos")
              reitross = reitross % 500
            if (reitross >= 200):
              queda = reitross//200
              print(str(queda) + "moneda\s de 200 pesos")
              reitross = reitross % 200
            if (reitross >= 100):
              queda = reitross//100
              print(str(queda) + "moneda\s de 100 pesos")
              reitross = reitross % 100
            if (reitross >= 10):
              queda = reitross//10
              print(str(queda) + "moneda\s de 10 pesos")
              reitross = reitross() % 10
            cajaresultado=dinero_de_caja-reitross
            print("Lo que queda en la caja es: ", cajaresultado)
